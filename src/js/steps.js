...
const endPoint = _ => ''
const getAttributes = next => (item, opt) =>
  (item.attributes ? ' ' + item.attributes.join(' ') : '') + next(item, opt)
const getValue = (item, opt) =>
  opt[item.name] != null ? opt[item.name] : item.value
const getClassAttr = classes =>
  classes && classes.length ? ` class="${classes.join(' ')}"` : ''
const getClassList = classes =>
  classes && classes.length ? ' ' + classes.join(' ') : ''
const getValueLabel = next => (item, opt) =>
  item.text ? `<span class="opValue">${item.text}${next(item, opt)}</span>` : ''
const getOptionLabel = next => (item, opt) =>
  next(item, opt) +
  (item.text ? `<span class="opValue">${item.text}</span>` : '')
const getDisabled = next => (item, opt) =>
  (item.disabled ? ' disabled' : '') + next(item, opt)
const isChecked = opt => item =>
  (!opt[item.name] && item.checked) ||
  opt[item.name] === item.value ||
  (opt[item.name] && opt[item.name][item.value])
const getChecked = next => (item, opt) =>
  (isChecked(opt)(item) ? ' checked' : '') + next(item, opt)
const getOptions = item =>
  item.options
    ? item.options
      .map(
        o =>
          `<option value="${o.value}"${getDisabled(endPoint)(o)}>${o.value
          }</option>`
      )
      .join('')
    : ''
...
const getFileTag = next => (item, opt) =>
  `<input type="file" name="${item.name}" ${getClassAttr(item.classes)}${next(
    item,
    opt
  )}>`
const getSelectTag = next => (item, opt) =>
  `<select id="${item.name}" name="${item.name}" ${getClassAttr(
    item.classes
  )}${next(item, opt)}>${getOptions(item)}</select>`
const getTextAreaTag = (next, getValue = item => item.value) => (item, opt) =>
  `<textarea id="${item.name}" name="${item.name}" ${getClassAttr(
    item.classes
  )}${next(item, opt)}>${getValue(item, opt)}</textarea>`
const getInputView = next => (item, opt) =>
  item.view ? item.view(next(item, opt)) : next(item, opt)
const getInputText = getInputView(
  getInputTag(getDisabled(getAttributes(endPoint)), getValue)
)
const getBindHTML = opt => item => getArgHTML(item, opt, false)
const getInputBinds = next => (item, opt) =>
  next(item, opt) +
  (item.binds ? item.binds.map(getBindHTML(opt)).join('') : '')
const getInputCheckOption = reduceApply(
  endPoint,
  getAttributes,
  getDisabled,
  getChecked,
...
const getListElement = (item, opt) => {
  var listId = `list-${item.name}`,
    attr = `list="${listId}"`
  item.attributes ? item.attributes.push(attr) : (item.attributes = [attr])
  return [
    getInputText(item, opt),
    `<datalist id="${listId}">`,
    getOptions(item),
    '</datalist>',
    getInputBinds(endPoint)(item, opt)
  ].join('')
}
...
const elementTypeMapping = {
  ...
  select: getInputBinds(
    getInputView(getSelectTag(getDisabled(getAttributes(endPoint))))
  ),
  list: getListElement
}
...
const getPanelView = next => (panel, opt, pos) =>
  getPanelTitle(pos, panel.text) + next(panel, opt)
const getPanelInnerView = next => (panel, opt) =>
  '<hr>' + getDiv(x => x)(panel.description) + next(panel.args, opt)
const getIndexItemHTML = name =>
  `<a class="btn btn-lg" name="${name}">${panels[name].text}</a>`
...
const getIndicatorHTML = (pos, condition) =>
  condition ? `<hr class="drag-indicator" data-position=${pos}>` : ''
const getSelected = selected => (selected ? ' selected' : '')
const getDraggable = step => (step.panel.draggable ? ' draggable' : '')
const getDelete = step =>
  step.panel.draggable ? `<a href="#" class="delete">${texts.delete}</a>` : ''
const getStepOpt = step => {
  var panel = step.panel,
    opt = Object.assign({}, step.opt),
    args = panel.args,
    res = []
  for (let key in opt)
    if (optionType[args[key].type]) {
      let f =
        args[key].type === 'radio'
          ? item => item.value.toString() === opt[key].toString()
          : item => opt[key][item.value.toString()]
      opt[key] = args[key].values
        .filter(f)
        .map(item => item.text || opt[key])
        .join(',')
    }
  if (panel.view) opt = panel.view(opt)
  for (let key in opt)
    res.push(
      getValueLabel((_, opt) => texts.labelSplitter + opt[key])(args[key], opt)
    )
  return res.join('')
}
...
const getPureStepHTML = (step, pos, selected = 0) =>
  `<div class="step${getSelected(selected)}" data-position=${pos}${getDraggable(
    step
  )}>${getStepInnerHTML(step, pos)}</div>`
const getStepNIndicatorHTML = pos => (step, i) =>
  getIndicatorHTML(
    i,
    step.panel.draggable && (i === 0 || !steps[i - 1].panel.draggable)
  ) +
  (step == indexStep
    ? getIndexStepHTML(i)
    : getPureStepHTML(step, i, i === pos)) +
...
const newStep = (panel, option) => ({
  panel,
  opt: Object.assign({}, panel.initOpt, option)
})
const context = (steps => {
  var pos = 0,
    index,
  ...
  const setFeatures = features => {
    addibleFeatures = features.filter(
      name => panels[name].draggable && name !== 'index'
    )
    ...
    addibleFeatures.forEach(name =>
      listeners.click.push({
  ...
  const loadOptions = options => {
    ...
    for (
      let i = bottoms.length;
      i--;
      k -= loadStep(bottoms[i], options[k], bottomSteps, files[i])
    );
    for (; j <= k; j++) loadStep(panels[options[j].op], options[j], steps)
  ...
  const refreshPanel = (refreshStep = 1) => {
    ...
    options.html(
      getPanelView(
        panel === panels.index ? getIndexHTML : getPanelInnerView(getArgsHTML)
      )(panel, opt, pos)
    )
    for (let argName in args)
      args[argName].type === 'file' &&
        (options.find(`input[name=${argName}]`).get(0).files = opt[argName])
    for (let argName in args) args[argName].load && args[argName].load(step)
    refreshStep && context.refreshCurrentStep()
    eventTypes.forEach(type =>
      listeners[type].forEach(o => $(o.selector).on(type, o.f))
    )
  }
  const refreshCurrentStep = _ =>
...
const initListeners = _ => {
  $('#steps')
    .on('dragenter', '.drag-indicator', function () {
    ...
    .on('drop', '.drag-indicator', function (ev) {
      ev.stopPropagation()
      let posSrc = +ev.originalEvent.dataTransfer.getData('text'),
        posTarget = +this.getAttribute('data-position')
    ...
    .on('click', 'a.delete', function (ev) {
      ev.stopPropagation()
      context.removeStep(
        +this.parentNode.parentNode.getAttribute('data-position')
      )
    })
  ...
  $('#options')
    .on('mouseenter', 'a.btn', function () {
      let panelName = this.getAttribute('name')
      panels[panelName].description &&
        $('#description').html(panels[panelName].description)
    })
...
const serializeSteps = (toFile, data = new Map()) =>
  steps
    ...
    .map(step => {
      let opt = Object.assign({}, step.opt),
        panel = step.panel
      if (panel.submit) {
        for (let key in panel.args) {
          let arg = panel.args[key]
          if (
            (arg.type === 'number' || arg.dataType === 'number') &&
            opt[key] != null
          )
            opt[key] = +opt[key]
        }
...
const submit = data =>
  data.set('steps', JSON.stringify(serializeSteps(null, data)))
